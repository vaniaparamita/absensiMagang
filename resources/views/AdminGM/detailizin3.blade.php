        ...
        <div class="row mb-2">
          <div class="col-sm-12" style="background:white">
            ...
            <div class="content-header" style="background:#1675d1">
              <div class="card-title" style="font-size:18px;color:white">Detail Pengajuan Izin</div>
      
             </div>
            </div>
            <br />
            @foreach($data as $d)
            <form action="{{ route('detailizin_gm.update', $d->id) }}" method="post" enctype="multipart/form-data">
			{{ csrf_field() }}
			{{ method_field('PUT') }}
					...
					<div class="form-group">
					  	<label for="surat_cuti">Surat Izin :</label>
					...
					<div class="form-group">
					  	...
					  	<textarea class="form-control" id="alasan" name="alasan" placeholder="Isi Apabila Izin Ditolak"></textarea>
					</div>				 
					
					<div class="form-group">
					  <button type="submit" class="btn btn-md btn-primary" style="background-color:#1675d1">Simpan</button>
					  <a class="btn btn-danger" href="{{route('detailizin_gm.index')}}" role="button">Batalkan</a>
					</div>
				</form>
				@endforeach
            <br />
